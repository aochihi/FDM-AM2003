...

{
  ...
  total_size += ((nh1 - nl1 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh2 - nl2 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh3 - nl3 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh4 - nl4 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh5 - nl5 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh6 - nl6 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh7 - nl7 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh8 - nl8 + 1 + NR_END) * sizeof(double)) ;
  total_size += ((nh9 - nl9 + 1 + NR_END) * sizeof(double)) ;


  p = (double *) malloc (total_size) ;
//  if (!p) nrerror("error in allocate_communication_buffer ()");
...
initiate to 0 */
{
        ...
        v=(double *)calloc( (nh-nl+1+NR_END), (size_t) (sizeof(double)));
        if (!v) nrerror("allocation failure in dvector()");
        return v-nl+NR_END;
}
...
and initiate 0*/
{
        ...
        /* allocate pointers to rows */
        m=(double **) calloc( (nrow+NR_END), (size_t)(sizeof(double*)));
        if (!m) nrerror("allocation failure 1 in matrix()");
        m += NR_END;
        m -= nrl;

        /* allocate rows and set pointers to them */
        m[nrl]=(double *) calloc((nrow*ncol+NR_END), (size_t)(sizeof(double)));
        if (!m[nrl]) nrerror("allocation failure 2 in matrix()");
        m[nrl] += NR_END;
        m[nrl] -= ncl;
...
and initiate 0*/
{
        ...
        /* allocate pointers to pointers to rows */
        t=(double ***) calloc((nrow+NR_END), (size_t)(sizeof(double**)));
        if (!t) nrerror("allocation failure 1 in f3tensor()");
        t += NR_END;
        t -= nrl;

        /* allocate pointers to rows and set pointers to them */
        t[nrl]=(double **) calloc((nrow*ncol+NR_END), (size_t)(sizeof(double*)));
        if (!t[nrl]) nrerror("allocation failure 2 in f3tensor()");
        t[nrl] += NR_END;
        t[nrl] -= ncl;

        /* allocate rows and set pointers to them */
        t[nrl][ncl]=(double *) calloc((nrow*ncol*ndep+NR_END), (size_t)(sizeof(double)));
        if (!t[nrl][ncl]) nrerror("allocation failure 3 in f3tensor()");
        t[nrl][ncl] += NR_END;
        t[nrl][ncl] -= ndl;
